...
const Secretary = require("../models/Secretary");
const verifyToken = require("../middleware/verifyToken");
const isCompanySecretary = require("../middleware/isCompanySecretary"); // Keep this middleware for specific secretary actions

const router = express.Router();

// 1. **NEW ROUTE FOR CREATING SECRETARY FOR A SPECIFIC COMPANY**
//    This route will handle POST requests to /api/secretaries/:companyId
router.post("/:companyId", verifyToken, async (req, res) => {
    // Changed path to include :companyId
    const { name, email, contact } = req.body; // Remove 'id' as it's auto-generated by Mongoose
    const companyId = req.params.companyId; // Get companyId from URL parameters
    ...
    try {
        ...
        const newSecretary = new Secretary({
            name,
            // Mongoose automatically generates _id, no need for a separate 'id' field in schema unless it's a custom one.
            // If 'id' in your Secretary schema is meant to be a custom ID you provide, keep it, otherwise remove it.
            // For MongoDB _id, you don't pass it in the request body.
            email,
            contact,
        });
        ...
        // Add secretary to company's secretary list
        // The company is already found by companyId from req.params
        company.secretary.push(savedSecretary._id);
        await company.save();
        ...
        console.error(error);
        // Be more specific with errors, e.g., validation errors vs server errors
        res.status(400).json({ error: "Failed to create secretary" });
    }
});

// Existing GET ALL SECRETARIES FOR THE SELECTED COMPANY (UNCHANGED)
router.get("/secretaries", verifyToken, async (req, res) => {
    try {
        const selectedCompany = req.header("selectedCompany");
...
// Existing GET A SPECIFIC SECRETARY BY ID (Uses isCompanySecretary middleware - CORRECT USAGE here)
router.get(
    "/secretaries/:id",
    verifyToken,
    isCompanySecretary, // This middleware correctly applies here as 'id' refers to the secretary
    async (req, res) => {
        try {
            const secretary = await Secretary.findById(req.params.id);
            if (!secretary) {
...
// Existing UPDATE A SECRETARY BY ID (Uses isCompanySecretary middleware - CORRECT USAGE here)
router.put(
    "/secretaries/:id",
    verifyToken,
    isCompanySecretary, // This middleware correctly applies here
    async (req, res) => {
        const { name, email, contact } = req.body;
        try {
            const updatedSecretary = await Secretary.findByIdAndUpdate(
...
// Existing DELETE A SECRETARY BY ID AND REMOVE FROM COMPANY (Uses isCompanySecretary middleware - CORRECT USAGE here)
router.delete(
    "/secretaries/:id",
    verifyToken,
    isCompanySecretary, // This middleware correctly applies here
    async (req, res) => {
        try {
            const deletedSecretary = await Secretary.findByIdAndDelete(
                req.params.id
            );
            if (!deletedSecretary) {
                return res.status(404).json({ error: "Secretary not found" });
