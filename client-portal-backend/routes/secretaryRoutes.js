...
router.post("/secretaries", verifyToken, async (req, res) => {
    const { name, email, contact } = req.body;
    const companyId = req.header("selectedCompany"); // <- from header

    if (!companyId) {
        return res
            .status(400)
            .json({ error: "selectedCompany header is required" });
    }

    const company = await Company.findById(companyId);
    if (!company) return res.status(404).json({ error: "Company not found" });

    const savedSecretary = await new Secretary({ name, email, contact }).save();
    company.secretary.push(savedSecretary._id);
    await company.save();
    res.status(201).json(savedSecretary);
});
